...
public class Player1 implements Player {
    ...
    public Line makePlay(Board board, Line oppPlay, long timeRemaining) {
        ...
        Set<Square> marked2SideSquares = board.squaresWithMarkedSides(2);
        ...
        if (!marked3SidesSquares.isEmpty()) {
          ...
          for (Square square : marked3SidesSquares) {
            ...
            int r = square.getRow();
            int c = square.getCol();
            Set<Line> lineSet= square.openLines();
            if (Side.WEST == false) {
              Square nextSQ =  Board.getSquare(r, c-1);
              Set<Line> nextSQLines = nextSQ.openLines();
              for (Line lines : nextSQLines) {
                 chainC ++ = 1;
            if (chainC > highCount) {
                highCount = chainC;
                 }
            ...
            Set<Line> lineSets= square.openLines();
        ...
        Line line = chooseRandomLine(marked1SideSquares, board);
        if (line != null)
            return line;
        
        //get a line from any of the square that has no marked side
        line = chooseRandomLine(notMarkedSquares, board);
        if (line != null)
            return line;
